...
```{r}
#Before renewal rlv = mp(1+d)/1+d-p
rlvB <- (m*p*(1+d))/(1+d-p)
rlvB

#After renewel rlv = (m*p)/(1+d-p)
rlvA <- (m*p)/(1+d-p)
rlvA
...
## actual data is backward looking, you make a model to make predictions/forecast clv/rlv
...
## doens not capture the less loyal customer leaving earlier and the more loyal customers staying longer --> lower clv/rlv for the predicted model
